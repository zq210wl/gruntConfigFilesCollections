module.exports = function(grunt) {
  ...
  grunt
    .initConfig({
      ...
      meta: {
        ...
        distDir: "core",
        sourceDir: 'resource',
        sourceTemplates: '<%= meta.sourceDir %>/templates'
      },
      /**** clean work *****/
      clean: {
        options:{
      ...
      html2js: {
        all:{
          options:{
            ...
            base: '<%= meta.sourceDir %>'
            //base option is the base root of templeate files, the generated path in clarity.tpl.js will not contain the bose root. Only 'like templates/address/address.html'
          },
          src:['<%= meta.sourceTemplates %>/**/*.html'],
          dest:'<%= meta.distDir %>/scripts/clarity.tpl.js'
        }
      },
      /**** notify work ****/
      notify: {
      ...
      /* esteWatch work, just for development envirement*/
      esteWatch: {
          options: {
            livereload: {
              enabled: false
          ...
          "*": function(filepath) {
                //exclude watching for the temporary files like ~RFa63ce6b.TMP and .suble00.tmp
                var regForTMP = /.*?\.TMP$/igm;
                if(!regForTMP.test(filepath)){
                  ...
                  //config copy task for single file changed.
                  var changedFileConfig = [{
                    expand: true,
                    cwd: dirname+'/',
                    src: basename,
                    dest: dirname.replace(resource, dist)+'/'
                  }];
                  grunt.config(['copy', 'changedFile', 'files'], changedFileConfig);

                  //config notify task for single file changed.
                  var processChangedFile = {
                    title: 'Process success',
                    message: 'Processed file "'+basename+'" successfully.'
                  };
                  grunt.config(['notify', 'processChangedFile', 'options'], processChangedFile);

                  //check if the changed file is template, if it is, will recompile templates to js file.
                  var templatesPath = grunt.config('meta.sourceTemplates');
                  var templatesPathR = templatesPath.replace(/\//igm,"\\/");
                  var regForTemplates = new RegExp('^('+templatesPathR+')\/.+','igm');
                  if(regForTemplates.test(filepath)){
                    //config clean task for single file changed.
                    grunt.config(['clean', 'templatesJsFile'], grunt.config('html2js.all.dest'));
                    returnVal = ['clean:templatesJsFile', 'copy:changedFile', 'html2js:all', 'notify:processChangedFile'];
                  }else{
                    //config clean task for single file changed.
                    grunt.config(['clean', 'changedFile'], filepath.replace(resource, dist));
                    returnVal = ['clean:changedFile', 'copy:changedFile', 'notify:processChangedFile'];
                  }
                  return returnVal;
                }
              }
  ...
  // Default task(s). used for development envirment
  grunt.registerTask('default', ['clean:start', 'copy:development', 'html2js:all', 'notify:development', 'esteWatch']);
